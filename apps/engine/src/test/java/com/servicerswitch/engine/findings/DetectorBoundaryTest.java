...
class DetectorBoundaryTest {
  ...
  private final PropertyTaxProjectionMismatchDetector taxDetector =
      new PropertyTaxProjectionMismatchDetector();
  private final EscrowShortageCalculationErrorDetector shortageDetector =
      new EscrowShortageCalculationErrorDetector();
  private final DuplicateTaxDisbursementDetector duplicateDetector =
      new DuplicateTaxDisbursementDetector();
  private final UnexplainedPaymentIncreaseDetector paymentDetector =
  ...
  void balanceDetectorFiresAboveTolerance() {
    MortgageAccount account =
        account(
            analysis(
                LocalDate.of(2024, 5, 31), amount("1000.00"), amount("6000.00"), amount("400.00")),
  ...
  void shortageDetectorFiresAboveTolerance() {
    assertThat(
            shortageDetector.detect(
                context(
                    withStatedShortage("410.01"),
                    amount("400.00"),
    ...
    return account(
        ...
        List.of(
            new EscrowTransaction(
                firstDate,
                TransactionType.TAX_DISBURSEMENT,
                amount("-1000.00"),
                "County",
                amount("0.00")),
            new EscrowTransaction(
                firstDate.plusDays(dayDifference),
                TransactionType.TAX_DISBURSEMENT,
                amount("-" + secondAmount),
                "County",
                amount("0.00"))));
