...
public final class PaymentCalculator {
    public PaymentDecomposition decompose(MortgageAccount account, LocalDate transferDate) {
        Payment oldPayment = account.payments().stream()
        ...
        Payment newPayment = account.payments().stream()
                .filter(payment -> !payment.date().isBefore(transferDate))
        ...
        BigDecimal principalInterestChange = cents(
                newPayment.principal().add(newPayment.interest())
                        .subtract(oldPayment.principal().add(oldPayment.interest())));
        BigDecimal taxChange = divide(
                newAnalysis.projectedAnnualTax().subtract(oldAnalysis.projectedAnnualTax()),
                TWELVE);
        BigDecimal insuranceChange = divide(
                newAnalysis.projectedAnnualInsurance()
                        .subtract(oldAnalysis.projectedAnnualInsurance()),
                TWELVE);
        BigDecimal residual = cents(paymentChange
        ...
        BigDecimal tolerance = maximum(
                Tolerances.PAYMENT_ABSOLUTE,
                cents(paymentChange.multiply(Tolerances.PAYMENT_PERCENT)));
        FindingType outcome = residual.compareTo(tolerance) > 0
